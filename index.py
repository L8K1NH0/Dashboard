                ...
                dbc.CardBody([
                    ...
                    dbc.Row([
                        dbc.Col([
                            ...
                            html.Legend("Stallos Tecnologia")
                        ])
                    ], style={'margin-top': '10px'}),
                    dbc.Row([
                        dbc.Button("Visite o Site", href="https://stallos.com.br/#sobre", target="_blank")
                    ], style={'margin-top': '10px'})
                ])
            ], style=tab_card)
        ], sm=4, lg=2),
        ...
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    ...
                    html.Div(id='team-select', style={'text-aling':'center', 'margin-top': '30px'}, className='dbc')
                ])
            ],style=tab_card)
        ],sm=12, lg=2),
    ],className='g-2 my-auto', style={'margin-top': '7px'})
...
def graph3(team, toggle):
    ...
    fig3.add_annotation(text='Chamadas Realizadas por dia do Mês',
...
def graph4(team, toggle):
    ...
    fig4.add_annotation(text='Chamadas Realizadas por Mês',
        xref="paper", yref="paper",
        font=dict(size=17,
        color= 'gray'
        ),
...
def graph5(month, toggle):
    ...
    fig6.add_trace(go.Indicator(mode='number+delta',
        title = {"text": f"<span style='font-size:95%'>{df_5['Equipe'].iloc[0]}- Top Equipe</span>"},
        #title = {"text": f"<span>{df_5['Equipe'].iloc[0]}- Top Team</span><span style='font-size:70%'>Em Vendas - em relação a média</span>"},
        value = df_5['Valor Pago'].iloc[0],
        number = {'prefix':"R$"},
        delta = {'relative': True, 'valueformat': '.1%', 'reference': df_5['Valor Pago'].mean()}
    ))
